...
class WordPress_API {
    ...
    private function fetch_plugins_by_author() {
        ...
        if ( is_wp_error( $response ) ) {
            error_log( 'BLST WordPress API Error: ' . $response->get_error_message() );
            return array();
        }

        $code = wp_remote_retrieve_response_code( $response );
        if ( $code !== 200 ) {
            error_log( 'BLST WordPress API HTTP Error: ' . $code );
            return array();
        }
        ...
        if ( json_last_error() !== JSON_ERROR_NONE ) {
            error_log( 'BLST WordPress API JSON Error: ' . json_last_error_msg() );
            return array();
        }
