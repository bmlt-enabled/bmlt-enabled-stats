...
<div class="blst-stats-container blst-theme-<?php echo esc_attr( $theme ); ?>">
	...
	<section class="blst-section blst-github-section">
		...
		foreach ( $github_stats['top_repos'] ?? array() as $repo ) {
		...
		?>
		<?php if ( $has_open_issues || ! empty( $github_stats['closed_issues_by_repo'] ) ) : ?>
		<div class="blst-charts-row">
			...
			<?php endif; ?>
			<?php if ( ! empty( $github_stats['closed_issues_by_repo'] ) ) : ?>
			<div class="blst-chart-container blst-chart-fixed-height">
		...
		<?php endif; ?>
		<?php if ( ! empty( $github_stats['top_contributors'] ) ) : ?>
		<div class="blst-charts-row">
			<div class="blst-chart-container blst-chart-fixed-height">
...
$repos_by_issues = $github_stats['top_repos'] ?? array();
usort(
	$repos_by_issues,
	function ( $a, $b ) {
...
$closed_by_repo        = $github_stats['closed_issues_by_repo'] ?? array();
...
echo wp_json_encode(
	array(
		'github'    => array(
			...
			'contributors'       => array_column( $github_stats['top_contributors'] ?? array(), 'login' ),
			'commits'            => array_column( $github_stats['top_contributors'] ?? array(), 'commits' ),
			'additions'          => array_column( $github_stats['top_contributors'] ?? array(), 'additions' ),
			'deletions'          => array_column( $github_stats['top_contributors'] ?? array(), 'deletions' ),
		),
		'wordpress' => array(
