...
<div class="blst-stats-container blst-wordpress-only blst-theme-<?php echo esc_attr( $theme ); ?>">
    ...
    <section class="blst-section blst-wordpress-section">
        ...
        <div class="blst-plugin-grid">
            <?php foreach ( $stats['plugins'] as $plugin ) : ?>
            <div class="blst-plugin-card">
                <div class="blst-plugin-header">
                    <h3 class="blst-plugin-name"><?php echo esc_html( $plugin['display_name'] ); ?></h3>
                    <?php if ( ! empty( $plugin['version'] ) ) : ?>
                    <span class="blst-plugin-version">v<?php echo esc_html( $plugin['version'] ); ?></span>
                    <?php endif; ?>
                </div>

                <?php if ( ! empty( $plugin['short_description'] ) ) : ?>
                <p class="blst-plugin-desc"><?php echo esc_html( wp_trim_words( $plugin['short_description'], 12 ) ); ?></p>
                <?php endif; ?>

                <?php if ( ! empty( $plugin['rating'] ) ) : ?>
                <div class="blst-plugin-rating">
                    <?php echo wp_kses_post( Shortcodes::render_stars( $plugin['rating'] ) ); ?>
                </div>
                <?php endif; ?>

                <div class="blst-plugin-stats">
                    <div class="blst-plugin-stat">
                        <span class="blst-stat-value"><?php echo esc_html( Shortcodes::format_number( $plugin['active_installs'] ?? 0 ) ); ?>+</span>
                        <span class="blst-stat-label"><?php esc_html_e( 'Active', 'bmlt-enabled-stats' ); ?></span>
                    </div>
                    <div class="blst-plugin-stat">
                        <span class="blst-stat-value"><?php echo esc_html( Shortcodes::format_number( $plugin['downloaded'] ?? 0 ) ); ?></span>
                        <span class="blst-stat-label"><?php esc_html_e( 'Downloads', 'bmlt-enabled-stats' ); ?></span>
                    </div>
                </div>

                <div class="blst-plugin-bar">
                    <div class="blst-bar-fill" style="width: <?php echo esc_attr( min( 100, ( ( $plugin['active_installs'] ?? 0 ) / max( 1, $stats['total_active_installs'] ) ) * 200 ) ); ?>%"></div>
                </div>
                ...
                <p class="blst-plugin-updated">
                    ...
                    printf(
                        ...
                        esc_html( Shortcodes::format_date( $plugin['last_updated'] ) )
                    );
                    ?>
                </p>
                <?php endif; ?>

                <a href="<?php echo esc_url( $plugin['plugin_url'] ?? '#' ); ?>" class="blst-plugin-link" target="_blank" rel="noopener noreferrer">
